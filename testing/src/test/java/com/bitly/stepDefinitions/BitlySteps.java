...
public class BitlySteps extends BitlyBaseSteps implements En {
    ...
    public BitlySteps() {
        ...
        When("^sort bitlinks using \"([^\"]*)\" for group id \"([^\"]*)\" of first group \"([^\"]*)\"$", (String sort, String groupId, String jsonPathOfFirstGroup, DataTable responseData) -> {
            ...
            List<SortInput> responseClass = responseData.asList(SortInput.class);
            Map<String, String> sortParams = oMapper.convertValue(responseClass.get(0), Map.class);
            String body = scenarioContext.getLastResponse().getBody();
            assertThatJson(body).inPath(jsonPathOfFirstGroup).isNotNull();
            scenarioContext.setContext(groupId, getNodeValue(body, jsonPathOfFirstGroup));
            ExtractedResponse responseByGroupId = bitlyClient.getSortedBitlinksForGroupId(scenarioContext.getContext(groupId).toString(),sort,sortParams ,scenarioContext.getContext(Constants.ACCESS_TOKEN).toString());
            scenarioContext.setLastResponse(responseByGroupId);
        });

//        When("^sort bitlinks using \"([^\"]*)\" for group id \"([^\"]*)\" of first group \"([^\"]*)\"$", (String sort, String groupId, String jsonPathOfFirstGroup) -> {
//            String body = scenarioContext.getLastResponse().getBody();
//            assertThatJson(body).inPath(jsonPathOfFirstGroup).isNotNull();
//            scenarioContext.setContext(groupId, getNodeValue(body, jsonPathOfFirstGroup));
//            ExtractedResponse responseByGroupId = bitlyClient.getSortedBitlinksForGroupId(scenarioContext.getContext(groupId).toString(),sort ,scenarioContext.getContext(Constants.ACCESS_TOKEN).toString());
//            scenarioContext.setLastResponse(responseByGroupId);
//        });
    }
}
